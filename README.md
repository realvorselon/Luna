...
A separate static preview, `prototype.html`, is quietly linked from Project Trail and shows the first one-project Luna slice with sample public-safe placeholder data. It now includes tiny local browser saving for only the project name and current goal fields. A human tested those two saved values and confirmed they survive refresh. They save with localStorage in the same browser, do not sync, do not create an account, and can be cleared by clearing browser site data. Current Mode, last completed run, Next Action, Ignore For Now, Record the Change, rest / next step, and every other card are not saved yet. A new clear saved values boundary note defines that any later first clear control should clear only those two localStorage values and remain clearly local to the browser. Luna should rest before implementing clear/reset controls, saving more fields, sessionStorage, cookies, backend, accounts, database, sync, dashboard expansion, visual redesign, mode workflow, multi-project system, or more editable cards.
