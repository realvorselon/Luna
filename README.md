...
The current single-project browser prototype phase is complete and closed. The active phase is **Guided Return**: a small exploration of whether Luna can feel less like a form and more like a calm path someone moves through while preserving the proven one-project return loop. The boundary for this phase lives in `docs/GUIDED_RETURN_PHASE_BOUNDARY.md`. The first implementation slice adds one optional, reversible guided view in `prototype.html` for only Orient → Remember → Choose, and its first human test passed technically: forward navigation, backward navigation, returning to the full overview, and preserved editable values all worked. A tiny CSS-only follow-up now gives the guided controls and clear-local-edits button a slightly stronger, darker underside shadow so they stand apart from the card surface while keeping the same calm moonlit palette, pill shape, gradients, borders, typography, and focus treatment. The guided view still does not add Set Aside, Record, Rest, new persistence, task lists, checklists, or broader workflow behavior. The next suggested step is a human visual test of whether those controls now stand apart clearly enough while remaining calm and moonlit.
