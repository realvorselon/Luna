...
The current single-project browser prototype phase is complete and closed. The active phase is **Guided Return**: a small exploration of whether Luna can feel less like a form and more like a calm path someone moves through while preserving the proven one-project return loop. The boundary for this phase lives in `docs/GUIDED_RETURN_PHASE_BOUNDARY.md`. The first implementation slice now adds one optional, reversible guided view in `prototype.html` for only Orient → Remember → Choose. The full overview remains available as the proven reference mode, and the guided view does not add Set Aside, Record, Rest, new persistence, task lists, checklists, or broader workflow behavior. The next suggested step is a human test of guided navigation, reversibility, preserved editable values, and whether the guided view feels calmer than the full-page form-like overview.
