...
A separate static preview, `prototype.html`, is quietly linked from Project Trail and shows the first one-project Luna slice with sample public-safe placeholder data. It now includes the first tiny editable prototype UI for temporary in-browser project name and current goal edits. A human checked that first editable prototype UI and it seemed okay. The edits update the visible prototype in the browser only; they do not save, use storage, or add forms, backend, accounts, database, mode workflow, dashboard expansion, visual redesign, or a multi-project system, and refreshing may reset the sample data. Luna now has a separate saving boundary note before any persistence work; saving is not implemented, and Luna should rest before implementing localStorage, saving, sessionStorage, cookies, backend, accounts, database, sync, dashboard expansion, visual redesign, mode workflow, multi-project system, or more editable cards.
