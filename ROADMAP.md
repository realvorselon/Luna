...
- Keep testing whether each Guided Return question screen contains the ask, answer, quiet Luna response, and one direct next step on mobile.
- Status remains Rest; next suggested step is: “Human mobile test of whether Guided Return feels lighter with inline Luna responses and no between-question screens.”
...
- Keep testing whether each Guided Return screen has one clear job on mobile.
...
- Keep Luna focused on one unfinished thing, one clear place to begin again, and a composed Return Card that reduces what the user has to carry.
